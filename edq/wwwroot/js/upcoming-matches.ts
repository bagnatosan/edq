...
document.addEventListener("DOMContentLoaded", () => {
    const upcomingMatchesList = document.getElementById("upcomingMatchesList") as HTMLDivElement | null;
    const matchesCountBadge = document.getElementById("matchesCountBadge") as HTMLSpanElement | null;

    if (!upcomingMatchesList || !matchesCountBadge) return;

    // Cargar partidos futuros por AJAX
    const loadUpcomingMatches = async (): Promise<void> => {
        try {
            const response = await fetch("/Match/GetUpcomingMatches");
            if (!response.ok) throw new Error("Error al obtener los partidos programados.");

            const data: UserUpcomingMatch[] = await response.json();
    ...
    const renderUpcomingList = (matches: UserUpcomingMatch[]): void => {
        if (!upcomingMatchesList || !matchesCountBadge) return;
