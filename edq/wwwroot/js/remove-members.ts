...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const membersRemoveList = document.getElementById("membersRemoveList") as HTMLDivElement | null;
    ...
    const confirmModalCard = document.getElementById("confirmModalCard") as HTMLDivElement | null;
    const confirmModalText = document.getElementById("confirmModalText") as HTMLParagraphElement | null;
    ...
    if (!groupIdInput || !groupNameTitle || !membersRemoveList || !membersCountBadge) return;

    const groupId = parseInt(groupIdInput.value);
    if (isNaN(groupId)) return;

    // Callback para manejar el borrado una vez confirmado
    let currentDeleteCallback: (() => void) | null = null;

    const hideConfirmModal = (): void => {
        if (!confirmModal || !confirmModalCard) return;
    ...
    const renderMembersList = (members: GroupMember[], creatorId: number): void => {
        if (!membersRemoveList || !membersCountBadge) return;
        ...
        members.forEach(member => {
            ...
            if (!isCreatorSelf) {
                const deleteBtn = row.querySelector(".btn-action-delete") as HTMLButtonElement | null;
                if (deleteBtn) {
    ...
    const handleRemoveMember = (playerId: number, playerName: string, rowElement: HTMLDivElement): void => {
        if (!confirmModal || !confirmModalText) return;
        ...
        currentDeleteCallback = async () => {
            const deleteBtn = rowElement.querySelector(".btn-action-delete") as HTMLButtonElement | null;
